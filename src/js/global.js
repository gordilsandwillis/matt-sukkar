...
var global = {
  ...
  photoModal : function() {
    ...
    if ( global.isMobileDevice() ) {
      console.log('mobile photo modal open sequence');
      // mobile photo modal open sequence
  ...
  exploreSlideshow: function() {
    ...
    if ($("#explore-page-swipe")[0]){
      $("#explore-page-swipe").focus();
    }
    $("#explore-page-swipe").on("keydown", function(event) {
      if(event.keyCode == 37){
        if (!$('.explore-overlay').hasClass('hidden')){
          $('.explore-overlay').addClass('hidden');
        } else {
  ...
  photographySlideshow: function() {
    ...
    if ( global.isMobileDevice() ){
      ...
      // remove from dom
      if (document.getElementById('photo-modal-wrap')) {
        // photo-modal-wrap exists
        $('.photo-slideshow').remove();
        $('.arrow-nav').remove();
